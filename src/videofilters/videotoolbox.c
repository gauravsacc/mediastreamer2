...
typedef struct _VTH264EncCtx {
	...
	int rawFrameCount, encodedFrameCount;
} VTH264EncCtx;
...
static void h264_enc_output_cb(VTH264EncCtx *ctx, void *sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer) {
	...
	if(sampleBuffer == NULL || status != noErr) {
		...
		vth264enc_message("encoded frame #%d. time=%llu", ctx->encodedFrameCount, ctx->f->ticker->time);
		ctx->encodedFrameCount++;
	}
...
static void h264_enc_process(MSFilter *f) {
	...
	if ((frame = ms_queue_peek_last(f->inputs[0]))) {
		...
		if((err = VTCompressionSessionEncodeFrame(ctx->session, pixbuf, p_time, frame_duration, enc_param, NULL, NULL)) != noErr) {
			...
			vth264enc_message("passing raw picture #%d. time=%llu", ctx->rawFrameCount, f->ticker->time);
			ctx->rawFrameCount++;
		}
		CFRelease(pixbuf);
